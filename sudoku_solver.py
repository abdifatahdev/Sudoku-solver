class Sudoku:
    def __init__(self):
        self.board = [
            [7, 8, 0, 4, 0, 0, 1, 2, 0],
            [6, 0, 0, 0, 7, 5, 0, 0, 9],
            [0, 0, 0, 6, 0, 1, 0, 7, 8],
            [0, 0, 7, 0, 4, 0, 2, 6, 0],
            [0, 0, 1, 0, 5, 0, 9, 3, 0],
            [9, 0, 4, 0, 6, 0, 0, 0, 5],
            [0, 7, 0, 3, 0, 0, 0, 1, 2],
            [1, 2, 0, 0, 0, 7, 4, 0, 0],
            [0, 4, 9, 2, 0, 6, 0, 0, 7]
        ]

    def solve_sudoku(self, bo) -> None:
        find = self.find_next_empty(bo)
        if not find:
            return True
        else:
            row, col = find

        for i in range(1, 10):
            if self.is_valid(bo, i, (row, col)):
                bo[row][col] = i

                if self.solve_sudoku(bo):
                    return True

                bo[row][col] = 0

        return False

    def is_valid(self, bo, num, pos) -> bool:
        # Check row
        for i in range(len(bo[0])):
            if bo[pos[0]][i] == num and pos[1] != i:
                return False

        # Check column
        for i in range(len(bo)):
            if bo[i][pos[1]] == num and pos[0] != i:
                return False

        # Check box
        box_x = pos[1] // 3
        box_y = pos[0] // 3

        for i in range(box_y*3, box_y*3 + 3):
            for j in range(box_x * 3, box_x*3 + 3):
                if bo[i][j] == num and (i, j) != pos:
                    return False

        return True

    def print_board(self, bo) -> None:
        for row in range(len(bo)):
            if row % 3 == 0 and row != 0:
                print("- - - - - - - - - - - - ")

            for col in range(len(bo[0])):
                if col % 3 == 0 and col != 0:
                    print(" | ", end="")

                if col == 8:
                    print(bo[row][col])
                else:
                    print(str(bo[row][col]) + " ", end="")

    def find_next_empty(self, bo) -> tuple:
        for row in range(len(bo)):
            for col in range(len(bo[0])):
                if bo[row][col] == 0:
                    return (row, col)

        return None


if __name__ == "__main__":
    sudoku = Sudoku()
    sudoku.print_board(sudoku.board)
    sudoku.solve_sudoku(sudoku.board)
    print("\n")
    print("Solution using backtracking algorithm:")
    print("_______________________")
    sudoku.print_board(sudoku.board)
